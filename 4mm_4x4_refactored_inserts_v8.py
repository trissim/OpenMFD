...
base_path="./designs/open_chamber/2compartment_duplicates_4x4_refactored_inserts_v8/"
design = "open"
wells_pos=3
well_rad=2
rows=1
...
taper_len = deg_taper_len(wells_3d_height,degrees)
#taper_len=wells_3d_height*math.tan(math.radians(degrees))
taper_len_out=taper_len+taper_len_out_extra
(wells_many_single_top,_),_,_=make_device(design=design,wells_pos=wells_pos,
...
degrees = degrees_in
taper_len = deg_taper_len(wells_3d_height,degrees_in)
#taper_len=wells_3d_height*math.tan(math.radians(degrees))
taper_len_in=taper_len+taper_len_in_extra
(empty_top,_),_,_ = make_device(design=design,wells_pos=wells_pos-chan_l/2,
                                              well_rad=pin_dims_outer,chan_l=chan_l,chan_w=chan_w,chan_gap=chan_gap,num_chans=num_chans,
                                              rows=rows,columns=columns,casing_x=casing_x,casing_y=casing_y,chamber_len_until=chamber_len_until,rotate_units=0,
                                              alignment=None,chamber_width=chamber_width,add_chambers=False)
...
if not degrees == 0:
    open_duplicates_4x4_empty_top=chamfer_extrude(wells_3d_height_in,degrees,segments=20)(open_duplicates_4x4_empty_top)
else:
    open_duplicates_4x4_empty_top=solid.linear_extrude(height=wells_3d_height_in)(open_duplicates_4x4_empty_top)
