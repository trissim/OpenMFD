...
Changes to gaps, margins, taper allowances, and adhesive contact areas require
testing to determine which values work reliably.

These dimensions connect the printed and photolithographic parts of the mold.
Shared coordinates place the pins over their matching holes, while their sizes
set the alignment clearance. The insert skirt provides space for adhesive, and
taper affects both demolding and well capacity. The same shrinkage correction
is applied to the masks and inserts to preserve their relative positions and sizes.

| Design feature | Value used here | Allowance included in the design | Changes that require testing |
|---|---:|---|---|
...
| Insert seating across the array | Measured pin-height variation of 116 µm between the lowest and highest pins; compressible EPDM rubber sheet used during clamping | The clamping layers must accommodate at least the measured height variation so all pins can seat before the epoxy cures | Larger height variation across a print, stiffer clamping layers, or omission or replacement of the compressible layer |
...
| Well size, depth, and working volume | Safe maximum working volume of 50 µL per well in the present plate | At the fixed 9 mm spacing, taper and interwell wall thickness constrain well width and depth. The array fits the selected 110 × 74 mm glass sheet; enlarging the wells would require larger, substantially more expensive custom-cut glass | Applications requiring more than 50 µL per well: revise well geometry and check demolding, array and glass dimensions, handling, and cost |
...
| Parylene coating and channel dimensions | 1 µm parylene C; nominal channel width of 10 µm before coating | A uniform coating adds approximately 2 µm to ridge width. Relative ridge height remains approximately unchanged because the ridge top and surrounding base both receive coating. Final cast dimensions also depend on PDMS shrinkage | Measure coated ridge and cast dimensions to establish a coating-thickness correction; this correction is not yet included in the generator |

## S3B. Fabrication checks for the Figure 5 layouts

For each literature-inspired layout in Figure 5, OpenMFD generated corresponding
mask, insert, wafer, and frame files using the same method as for the neuronal device.
These layouts have not been fabricated or experimentally validated. Their
dimensions determine the first-fabrication checks listed below.

| Layout | Device spacing and array | Well radius | Microchannel summary | What was tested | Checks when first fabricating the design |
|---|---|---:|---|---|---|
