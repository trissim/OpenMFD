...
## Abstract
High-throughput microfluidic screening remains difficult for many academic labs because plate-format devices require both micron-scale channels and millimeter-scale wells, a combination that standard SU-8 workflows handle poorly and that often leads to manual PDMS punching. Here we present a hybrid mold-fabrication method that combines SU-8 photolithography for fine microfeatures with adhesively bonded resin 3D-printed inserts for tall well features. The inserts are bonded directly to the wafer mold, so wells are cast in place during PDMS curing rather than added afterward. After photolithography, the remaining workflow - resin printing, insert bonding, PDMS casting, and device assembly - uses standard benchtop equipment. Using low-viscosity epoxy and a lock-and-key alignment scheme, we integrate millimeter-to-centimeter well structures with micrometer-scale SU-8 features on a single mold. The resulting devices adopt a 96-well-format footprint and are compatible with multichannel liquid handling and plate-based imaging. We also provide an open-source layout generator, OpenMFD, which encodes the design rules needed for this workflow and exports fabrication-ready files. As a demonstration, we fabricate and validate a compartmentalized neuronal culture platform that supports long-term axon extension and retrograde cholera toxin B tracing in a plate-format PDMS device.
