...
import pudb
version=25
base_path="./designs/open_chamber/2_compartment_384_well_300um_suex100_v"+str(version)+"/"
device_name="2_compartment_384_well_300um_suex100_v"+str(version)
...
chan_l=0.3
chan_l_extra=2
num_chans=int(well_rad[0]/2.0/(chan_gap+chan_w))
chamber_len_until=wells_pos/2.0
chamber_width=well_rad[0]
...
glass_error=4
wall_thickness=.0001
outline_alignment_thickness=.0001
...
def make_well_insert(make_fun,degrees,height,add_chambers,taper_len_extra=0,well_rad=well_rad,chan_l=chan_l):
    ...
    try:
        iterator = iter(well_rad)
    except TypeError:
        pass
    else:
        import pudb
        #pu.db
        #well_rad=well_rad[0]/2.0
        chamber_width_edit = well_rad[0] - taper_len*2.0
        well_rad = [well_rad[0]-taper_len*2,well_rad[0]-taper_len*2]
...
#        well_rad = well_rad-taper_len
    (inserts,_),_,_ = make_fun(well_rad=well_rad,chan_l=chan_l_edit,
                               chamber_width=chamber_width_edit,
