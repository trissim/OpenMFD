...
import pudb
version=22
base_path="./designs/open_chamber/2_compartment_192_well_300um_v"+str(version)+"/"
device_name="2_compartment_96_well_300um_v"+str(version)
...
taper_len_out_extra=0.22
taper_len_in_extra=0.91
pin_height=0.1
pin_inner_height=2
taper_height=0
degrees_taper=0
chamber_hole_dims=(1.5,1.5)
pin_dims=(1.3,1.3)
...
make_chambers = partial(make_device,design=design,wells_pos=wells_pos,
                                         ...
                                         rows=rows,columns=columns,casing_x=casing_x,casing_y=casing_y,chamber_len_until=chamber_len_until,rotate_units=rotate_units,
...
text = solid.union()(text1,solid.translate([0,-4])(text2))
...
text = solid.translate([0,-50])(text)
"""MAKE MULTI VERSION"""
#channels
channels_array = make_unit_array(channels_many_single,dims,grid_size, dxf=True, alignment = "full",units_from_center=units_from_center,alignment_offset=alignment_offset,alignment_mark_size=alignment_mark_size)
#wells and chambers
wells_array = make_unit_array(chamber_wells_many_single,dims,grid_size, dxf=True, alignment = "hollow",units_from_center=units_from_center,alignment_offset=alignment_offset,alignment_mark_size=alignment_mark_size)
#add cure text
wells_array=solid.union()(wells_array,text)
#add water_wells
water_wells = make_water_wells(glass_size,glass_error,np.array(dims),grid_size)
water_wells = solid.translate([alignment_offset[0],alignment_offset[1]])(water_wells)
water_wells = solid.translate([grid_size[0]*dims[0]/2.0,grid_size[1]*dims[1]/2.0])(water_wells)
wells_array = solid.union()(wells_array,water_wells)
#add outline to wells and chambers
wells_array = solid.union()(wells_array,outline)
#both layers together
aligned_array = solid.union()(wells_array,channels_array)


channels_array=solid.scale([scale_percent,scale_percent])(channels_array)
wells_array=solid.scale([scale_percent,scale_percent])(wells_array)
aligned_array=solid.scale([scale_percent,scale_percent])(aligned_array)
...
#bottom layer
channels_array_final = add_wafer_to_mask(wafer_size,wafer_flat_len,channels_array,grid_size,dims,wafer_line_thickness=wafer_line_thickness,outer_mask_thickness=outer_mask_thickness,alignment_offset=alignment_offset,shrinkage_scale=scale_percent)
#top layer
wells_array_final = add_wafer_to_mask(wafer_size,wafer_flat_len,wells_array,grid_size,dims,wafer_line_thickness=wafer_line_thickness,outer_mask_thickness=outer_mask_thickness,alignment_offset=alignment_offset,shrinkage_scale=scale_percent)
#aligned
aligned_array_final = add_wafer_to_mask(wafer_size,wafer_flat_len,aligned_array,grid_size,dims,wafer_line_thickness=wafer_line_thickness,outer_mask_thickness=outer_mask_thickness,alignment_offset=alignment_offset,shrinkage_scale=scale_percent)

"""SAVE MULTI VERSION"""
save_model(channels_array_final,base_path,device_name+"_bottom")
save_model(wells_array_final,base_path,device_name+"_top")
save_model(aligned_array_final,base_path,device_name+"_aligned")
...
def make_well_insert_all(degrees_in,degrees_out,height_in,height_out,inner_well_rad,add_chambers=False):
    ...
    open_duplicates_4x4_wells=solid.translate([0,0,pin_height+skirt_height])(open_duplicates_4x4_wells)
...
inner_skirt=solid.offset(delta=-skirt_thickness)(outer_skirt)
skirt_multi=solid.difference()(outer_skirt,inner_skirt)
skirt_multi=solid.linear_extrude(height=skirt_height)(skirt_multi)
skirt_multi=solid.translate([0,0,pin_height])(skirt_multi)
...
#make pin 3D
chamber_insert_holes_extrude = solid.linear_extrude(height=pin_height+skirt_height+pin_inner_height)(chamber_insert_holes)
