...
def get_default_config():
    ...
    config = {
        ...
        "groove_width": 0.6,  # Width of the groove (gw)
        "groove_depth": 0.4,  # Depth of the groove (gd)
        "gap_device": 0.3     # Distance from device slot to groove
    }
...
def create_corners(config):
    ...
    plate_height_inner = config["plate_height_inner"]
    plate_height_diff = config["plate_height"] - plate_height_inner
    corner_x = config["corner_x"]
    corner_y = config["corner_y"]
...
def create_device_groove(config):
    ...
    groove = solid.translate([0, 0, plate_under_depth + groove_depth/2.0 -0.01])(groove)
...
def create_96_well_plate(config=None):
    ...
    #plate = solid.union()(plate, device_groove)
